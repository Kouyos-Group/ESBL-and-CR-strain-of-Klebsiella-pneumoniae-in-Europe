...
output[2,2] <- signif((0.5+atan(fitness_cost[1])/pi)*(0.15),4)*100
#Fitness cost CRK
output[3,1] <- "Fitness cost CRK"
output[3,2] <- signif((0.5+atan(fitness_cost[1])/pi)*(0.15)+(0.5+atan(fitness_cost[2])/pi)*(0.15),4)*100
#Import values
output[4,1] <- "Import of ESBL (reservoir size per 100000)"
output[5,1] <- "Import of ESBL (reservoir size per 100000)"
output[4,2] <- signif((0.5+atan(fitness_cost[3])/pi)*1000,4)
output[5,2] <- signif((0.5+atan(fitness_cost[4])/pi)*1000,4)
#helping calculation
alpha <-  (0.5+atan(fitness_cost[6])/pi) * 0.5
tau = 365/(3*(0.5 + alpha))
...
output[7,2] <- (0.5+atan(fitness_cost[40])/pi) * 4 * (1 - 0.5 - alpha)/(0.5 + alpha)
#Use this one if you want to calculate for the basic colonization, which differs from 20%
#base_c <- 0.2
#output[7,2] <- (0.5+atan(input$fitness_cost[40])/pi) * (1-base_c)/base_c * (1 - 0.5 - alpha)/(0.5 + alpha)
#Increased susceptibility by treatment
output[8,1] <- "Increased susceptibility by treatment (mu)"
output[8,2] <- signif((0.5+atan(fitness_cost[5])/pi),4)*100
...
for (c in 1:11){
  ...
  beta_tr2 <- (0.5+atan(fitness_cost[6+c])/pi)*50*beta_tr1
  output[9+order_c[c], 1] <-  paste("Hospital transmission rate in",input_names[c])
  output[9+order_c[c], 2] <-  signif((0.5+atan(fitness_cost[6+c])/pi)*50,6)
  output[9+11+order_c[c], 1] <-  paste("Initial prevelence of ESBL strain in",input_names[c])
  output[9+11+order_c[c], 2] <-  signif((0.5+atan(fitness_cost[6+11+c])/pi)*100,4)
  output[9+11+11+order_c[c], 1] <-  paste("Initial prevelence of CRK strain in",input_names[c])
  output[9+11+11+order_c[c], 2] <-  signif((0.5+atan(fitness_cost[6+11+11+c])/pi)*(0.5+atan(fitness_cost[6+11+c])/pi)*100,4)
}
